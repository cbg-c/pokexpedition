...
const SpriteDisplay = ({ id, isShiny, forceAnimated = false, noHover = false }: { id: number, isShiny: boolean, forceAnimated?: boolean, noHover?: boolean }) => {
  ...
  return <img src={getSpriteUrl(id, isShiny, animated)} alt="" className="pixel-sprite" draggable="false" onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)} />;
};
...
const PokemonSlot = ({ pos, p, isEnemy, isActive, onDragStart, onDrop }: { pos: number; p: any; isEnemy?: boolean; isActive?: boolean; onDragStart?: (p: Pokemon) => void; onDrop?: (pos: number) => void }) => {
  ...
  return (
    <div className={`party-slot ${isEnemy ? 'enemy-slot' : 'player-slot'} ${isActive ? 'active-fighter' : ''} ${!p ? 'empty-slot' : ''}`} draggable={!!p && !isEnemy} onDragStart={() => p && !isEnemy && onDragStart && onDragStart(p)} onDragOver={(e) => e.preventDefault()} onDrop={(e) => { e.preventDefault(); if (!isEnemy && onDrop) onDrop(pos); }}>
      {p && (
        <>
          <div className={`star-rating ${starClass}`}>★ {p.star === 3 || p.copies >= maxCopies ? 'Lv. MAX' : `Lv. ${p.copies}/${maxCopies}`}</div>
          <div className="field-type-badges">{p.types.map((t: string) => <div key={t} className={`field-type-badge bg-type-${t}`}>{t.substring(0, 3).toUpperCase()}</div>)}</div>
          <div className={`sprite-container ${p.status}`}>
            ...
            <div className="pokemon-tooltip">
              <h4>{p.name}</h4>
              <div className="tooltip-types">{p.types.map((t: string) => <span key={t} className={`tooltip-type bg-type-${t}`}>{t}</span>)}</div>
              <div className="tooltip-stats"><span>Atk {p.stats.attack}</span><span>Sp.A {p.stats.spAtk}</span><span>Def {p.stats.defense}</span><span>Sp.D {p.stats.spDef}</span><span>Spd {p.stats.speed}</span><span>HP {p.hp}/{p.maxHp}</span></div>
...
function App() {
  const { currentRegion, clearedRegions, setRegion, returnToMenu, hasSelectedStarter, isGameOver, selectStarter, playerTeam, enemyTeam, shopItems, gold, stage, level, xp, buyXp, isBattling, isFastForwarding, combatText, startBattle, toggleFastForward, gameTick, buyPokemon, refreshShop, swapSlots, resetGame, sellPokemon, shopFrozen, toggleFreeze, pokedex, highScores, pendingChoice, resolveChoice } = useGameStore();
  ...
  const activeBoard = playerTeam.filter(p => p.position < 6);
  const synergies = [...new Set(activeBoard.map(p => p.baseId))].reduce((acc, id) => {
    POKEMON_DB.find(p => p.baseId === id)?.types.forEach(t => acc[t] = (acc[t] || 0) + 1); return acc;
  }, {} as Record<string, number>);

  const activePlayer = activeBoard.slice().sort((a,b) => a.position - b.position).find(p => p.hp > 0);
    ...
    <div className={`game-wrapper ${isFastForwarding ? 'fast-forward' : ''}`}>
      {showPokedex && (
        <div className="modal-overlay" onClick={() => setShowPokedex(false)}>
          <div className="modal-box pokedex-modal" onClick={e => e.stopPropagation()}>
            <div className="pokedex-header"><h2>Pokedex</h2><button className="close-btn" onClick={() => setShowPokedex(false)}>X</button></div>
              ...
              {Array.from({ length: 151 }, (_, i) => i + 1).map(id => {
                ...
                return (
                  <div key={id} className={`pokedex-entry ${!isVisible ? 'unseen' : ''} ${isVisible && dexView === 'shiny' ? 'shiny-entry' : ''}`}>
                    ...
                    {isVisible ? <SpriteDisplay id={id} isShiny={dexView === 'shiny'} noHover={true} /> : <div className="unseen-placeholder">?</div>}
                    <span>#{id}</span>
                  </div>
                );
              })}
      ...
      <header className="main-header">
        <div className="title-group">
          ...
          <button onClick={toggleFastForward} className={`header-ff-btn ${isFastForwarding ? 'active' : ''}`}>Fast Forward: {isFastForwarding ? 'ON' : 'OFF'}</button>
        </div>
        <div className="header-info"><button className="support-btn">Support</button></div>
      </header>
      ...
      <main className="battle-area">
        ...
        <div className="battle-area-top">
          <div className="stage-tracker">
            ...
            <div className="high-score-text">Furthest Reached: Stage {highScores[currentRegion] || 1}</div>
        ...
        <div className="shop-layout">
          <div className="shop-cards">
            {shopItems.map((base, index) => {
              ...
              return (
                <div key={`${base.id}-${index}`} className={`shop-card tier-${base.tier} ${isOwned ? 'shop-card-owned' : ''} ${!canAfford ? 'disabled-card' : 'purchasable'}`} onClick={() => { if (canAfford) buyPokemon(index); }}>
                  <div className="shop-card-header"><div className="tier-ribbon">Tier {base.tier}</div>{isCaught && <div className="caught-icon" title="Caught" />}<div className="shop-type-badges">{base.types.map((t: string) => <div key={t} className={`type-badge bg-type-${t}`}>{t.toUpperCase()}</div>)}</div></div>
                  <div className="card-image-bg">{base.isShiny && <div className="shiny-sparkle-card">SHINY</div>}{isOwned && <div className="upgrade-badge">UPGRADE!</div>}<SpriteDisplay id={base.pokedexId} isShiny={base.isShiny} /></div>
                  <h3 className="shop-pokemon-name">{base.name}</h3>
                  <div className="card-stats-grid"><span title="Attack">Atk {base.stats.attack}</span><span title="Sp. Atk">Sp.A {base.stats.spAtk}</span><span title="Defense">Def {base.stats.defense}</span><span title="Sp. Def">Sp.D {base.stats.spDef}</span><span title="Speed">Spd {base.stats.speed}</span><span title="HP">HP {base.stats.hp}</span></div>

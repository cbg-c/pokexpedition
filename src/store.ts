...
const generateEnemies = (stage: number) => {
  if (stage === MAX_STAGE) return [{ id: 'boss', baseId: 150, pokedexId: 150, name: 'Mewtwo', type: 'psychic', tier: 4, stats: { hp: 500, attack: 150, defense: 100, spAtk: 200, spDef: 100, speed: 150 }, hp: 500, maxHp: 500, position: 0, status: 'idle' as const, copies: 9, star: 3 }];
  return Array.from({ length: Math.min(6, Math.ceil(stage / 3)) }, (_, i) => {
    ...
    return { baseId: base.id, pokedexId: dexId, name: NAMES[dexId], type: base.type, tier: base.tier, stats: { ...base.stats, speed: Math.floor(base.stats.speed * scale) }, id: `e-${Date.now()}-${i}`, hp: scaledHp, maxHp: scaledHp, position: i, status: 'idle' as const, copies: isStage2 ? 3 : 1, star: isStage2 ? 2 : 1 };
  });
};
...
export const useGameStore = create<GameState>((set, get) => ({
  ...
  enemyTeam: generateEnemies(1), shopItems: generateShop(1), gold: 12, stage: 1, isBattling: false, combatText: "",
  ...
  buyPokemon: (index) => set((s) => {
    ...
    if (existing) {
      ...
      const pTeam = s.playerTeam.map(p => {
        if (p.id === existing.id) {
          ...
          if (copies >= 3 && copies < 9) { star = 2; dexId = EVOS[p.baseId][0]; } 
          if (copies >= 9) { star = 3; dexId = EVOS[p.baseId][1]; } 
    ...
    return { gold: s.gold - cost, shopItems: newShop, playerTeam: [...s.playerTeam, { baseId: base.id, pokedexId: base.id, name: NAMES[base.id], type: base.type, tier: base.tier, stats: base.stats, id: Date.now().toString(), hp: base.stats.hp, maxHp: base.stats.hp, position, status: 'idle', copies: 1, star: 1 }] };
  })
}));
